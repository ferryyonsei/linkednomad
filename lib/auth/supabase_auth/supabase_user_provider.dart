...
class LinkedNomadSupabaseUser extends BaseAuthUser {
  LinkedNomadSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> linkedNomadSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = LinkedNomadSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
